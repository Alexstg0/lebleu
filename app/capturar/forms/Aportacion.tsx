...
export default function Aportacion({
  periodoId,
  socios,
  initial,
  onDone,
  ...
  const { busy, msg, submit } = useSubmit("/api/aportaciones", editing ? "PATCH" : "POST");
  ...
  return (
    <form className="form-card" onSubmit={onSubmit}>
      {!editing && <h2>Nueva aportación de socio</h2>}
      <div className="grid">
        <div className="field">
          <label>Fecha</label>
      ...
      <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 14 }}>
        <button className="btn" disabled={busy}>{busy ? "Guardando…" : editing ? "Actualizar" : "Guardar aportación"}</button>
